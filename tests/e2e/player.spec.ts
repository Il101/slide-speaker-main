...
test.describe('Player Functionality', () => {
  // Test all three pipeline modes
  const pipelines = ['classic', 'vision', 'hybrid'];
  
  for (const pipeline of pipelines) {
    test.describe(`${pipeline} pipeline`, () => {
      test(`should play lecture with ${pipeline} pipeline`, async ({ page }) => {
        // Navigate to the player page with pipeline parameter
        await page.goto(`/?lesson=demo&pipeline=${pipeline}`);
      ...
      test(`should display lecture text in ${pipeline} pipeline`, async ({ page }) => {
        // Navigate to the player page with pipeline parameter
        await page.goto(`/?lesson=demo&pipeline=${pipeline}`);
